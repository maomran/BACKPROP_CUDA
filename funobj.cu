...
float CrossEntropyLoss::getLoss(tensor* networkOutput, tensor* labels) {
    ...
    kSoftMaxCrossEntropyLoss<<<dimGrid, dimBlock>>>(networkOutput->DevData(), networkOutput->row, networkOutput->col, labels->DevData(), dError);
...
float CrossEntropyLoss::getAccuracy(tensor* networkOutput, tensor* labels) {
    ...
    dim3 dimBlock(32);
    dim3 dimGrid((networkOutput->col + dimBlock.x)/dimBlock.x);
    kSoftMaxCrossEntropyAccuracy<<<dimGrid, dimBlock>>>(networkOutput->DevData(), networkOutput->row, networkOutput->col, labels->DevData(), dAccuracy);
...
tensor* CrossEntropyLoss::calculate(tensor* networkOutput, tensor* labels, tensor* output) {
    dim3 dimBlock(32);
    dim3 dimGrid((networkOutput->col + dimBlock.x)/dimBlock.x);
    kSoftMaxCrossEntropy<<<dimGrid, dimBlock>>>(networkOutput->DevData(), networkOutput->row, networkOutput->col, labels->DevData(), output->DevData());
    return output;
}
