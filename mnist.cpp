...
tensor* MNISTDataSet::getBatchOfImages(int index, int size) {
    return new tensor(28*28, size, (float**)(this->images + index*size));
}

tensor* MNISTDataSet::getBatchOfLabels(int index, int size) {
    return new tensor(10, size, (float**)(this->labels + index*size));
}
